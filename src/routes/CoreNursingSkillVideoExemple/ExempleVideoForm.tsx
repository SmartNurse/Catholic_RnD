...
const ExempleVideoForm = (props: Props) => {
  const { disabled, user_id, patient_id, patient_name, totalSize, setTotalSize, refresh, setValue } = props;

  const { onSuccess, onFail, onResultCode, onRequired } = useNotification();

  const [files, setFiles] = useState<any[]>([{ saved: false, path: "", file: null }, { saved: false, path: "", file: null }, { saved: false, path: "", file: null }]);
  ...
  const deleteFile = (i: number) => {
    ...
    const newSize = Number((totalSize - files[i].file.size / Math.pow(1024, 3)).toFixed(4));
    setTotalSize(newSize < 0 ? 0 : newSize);
    const newFiles = [...files.slice(0, i), { saved: false, path: "", file: null }, ...files.slice(i+1)];
    setFiles(newFiles);
    setValue("files", newFiles);  
  }

  const downloadFile = (i: number) => {
    axios.get(`${process.env.REACT_APP_API_URL}/file/video_download?user_id=${user_id}&patient_id=${patient_id}&video_num=${i+1}`,
      {
        responseType: "blob"
      }
    )
  ...
  useEffect(() => {
    ...
    .catch((e) => {
      if (e.response.data.status !== 500) onFail('핵심간호술기영상을 불러오는 데 실패하였습니다.', e)
    })
  }, [refresh]);
              ...
              <MuiTextField
                ...
                onChange={(e) => {
                  ...
                  if (target && target.files) {
                    newFiles[i] = { saved: false, path: target.value, file: target.files[0] };
...
export default ExempleVideoForm;
