...
const OpInfo = (props: Props) => {
  ...
  const onDeleteCode = (idx: number) => {
    setInfos({
      ...infos,
      sub_code: [
        ...infos.sub_code.slice(0, idx),
        ...infos.sub_code.slice(idx + 1),
      ],
      sub_title: [
        ...infos.sub_title.slice(0, idx),
        ...infos.sub_title.slice(idx + 1),
      ],
    });
    setSubCount(subCount - 1);
  };
            ...
            <Fragment key={idx}>
              <RowContent title="부수술코드" titleRatio={1} childrenRatio={2}>
                <MuiTextField
                  ...
                  onChange={e => {
                    ...
                    newInfos.sub_code[idx]
                      ? (newInfos.sub_code[idx] = e.target.value)
                      : newInfos.sub_code.push(e.target.value);
                    setInfos({ ...newInfos });
              ...
              <RowContent title="부수술명" titleRatio={1} childrenRatio={2}>
                <MuiTextField
                  ...
                  onChange={e => {
                    const newInfos = { ...infos };
                    newInfos.sub_title[idx]
                      ? (newInfos.sub_title[idx] = e.target.value)
                      : newInfos.sub_title.push(e.target.value);
                    setInfos({ ...newInfos });
                  }}
                  required={false}
                  InputProps={{ endAdornment: <Search /> }}
