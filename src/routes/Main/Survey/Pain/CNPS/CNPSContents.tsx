...
const contentLabel = [
    {
        id: 1,
        ko: "얼굴 표정",
        desc: ["자연스러운 표정", "미간 찡그림, 눈살 찌푸림, 눈물 글썽거림", "눈물을 흘림, 눈을 꽉 감음, 입을 씰룩거리며 눈을 찡그림", "이를 악뭄, 기관 내관을 밀어내거나 깨뭄"],
    },
    {
        id: 2,
        ko: "신체 반응",
        desc: ["편안한 자세, 움직임이 없음", "느리고 조심스러운 움직임, 몸을 뒤척임", "통증 부위를 만지려고 하거나 문지름, 온몸에 힘을 줌", "온몸을 흔들거나 비틀며 심하게 움직임, 공격적 행동"],
    },
    {
        id: 3,
        ko: "기계 호흡",
        desc: ["경보가 울리지 않고, 잘 적응함", "경보가 울리지만 곧 멈춤", "경보가 자주 울림, 인공호흡기에 저항함", "기계 호흡과 Fighting"],
    },
    {
        id: 4,
        ko: "순응도 발성",
        desc: ["정상적인 말투", "공공대며 신음소리를 냄", "훌쩍거리거나, 소리를 내어 흐느껴 울음", "큰소리를 지름, 폭언을 함, 울부짖음"],
    },
];
const scoreLabel = [
    {score: "0", label: "통증없음"},
        ...
        <>
            <Box sx={{ width: "max-content", margin: "20px auto" }}>
                <Table aria-label="simple table">
                    <TableHead>
                        <TableRow>
                            <StyledTableCell colSpan={2} align="center" sx={{ padding: "16px" }}>지표</StyledTableCell>
                            <StyledTableCellWithoutRight align="center" sx={{ padding: "16px" }}>점수</StyledTableCellWithoutRight>
                            <StyledTableCellWithoutLeft colSpan={2} sx={{ padding: "16px" }}>설명</StyledTableCellWithoutLeft>
                        </TableRow>
                    </TableHead>
                    <TableBody>
                        {contentLabel.map((content: {id: number, ko: string, desc: string[]}, i) => (
                            <TableRow>
                                <StyledTableCell align="center" sx={{ padding: "16px", width: "62px" }}>{content.id}</StyledTableCell>
                                <StyledTableCell align="center" sx={{ padding: "16px", width: "200px" }}>{content.ko}</StyledTableCell>
